...
for blockNumber in range(1, 6):
    print("\n")
    print("Verifying Frames for Block " + str(blockNumber) + " Trailers, Please Wait")
    print("--------------------------------------------------\n")
    ...
    for x in movies:
        ...
        trailerLength = get_length(
            "./Movie-Trailers/Block_" + str(blockNumber) + "/" + x + ".avi"
        )
        imageCount = glob.glob(
            "./Movie-Trailers/" + "Block_" + str(blockNumber) + "/" + x + "/image*"
        )
...
if failCount > 0:
    if failCount == 1:
        ...
        print(
            ...
            + str(failCount)
            + " errors were detected, please check output above to view the errors\n\n"
        )
else:
